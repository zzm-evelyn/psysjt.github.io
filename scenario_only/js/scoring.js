...
function getAllResponses() {
  ...
  (data.questionnaire_responses || []).forEach(r => {
    ...
    if (!question) return;
    // text_input 题型不计入得分
    if (question.question_type === 'text_input') return;
...
function getGameResponsesForScore(gameKey) {
  const data = getParticipantData();
  const key = typeof normalizeGameKey === 'function' ? normalizeGameKey(gameKey) : (gameKey || 'game_a');
  return (data.game_responses || [])
    .filter(function (r) {
      const responseKey = typeof normalizeGameKey === 'function' ? normalizeGameKey(r.game_key) : (r.game_key || 'game_a');
    ...
    .map(function (r) {
      return {
        ...
        final_score: r.final_score !== undefined && r.final_score !== null && r.final_score !== 0 ? r.final_score : r.raw_score
      };
    });
}

function calculateGameScores(gameKey, gameTitle) {
  const key = typeof normalizeGameKey === 'function' ? normalizeGameKey(gameKey) : (gameKey || 'game_a');
  const title = gameTitle || (typeof defaultGameTitle === 'function' ? defaultGameTitle(key) : key);
  const allResponses = getGameResponsesForScore(key);
