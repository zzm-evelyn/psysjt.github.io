...
async function goToNext() {
  ...
  // 保存当前页所有答案到 localStorage（并尝试提交后端）
  await saveCurrentPageResponses();

  // 如果是最后一页 → 显示提交确认
  if (currentPageIndex === pageGroups.length - 1) {
    document.getElementById('questionnaireBody').style.display = 'none';
...
async function saveCurrentPageResponses() {
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (item.question_type === 'text_input') {
      ...
      responseData.raw_score = selectedOption ? selectedOption.score : sel.rawScore;
