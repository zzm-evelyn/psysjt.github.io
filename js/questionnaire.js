...
function applyQuestionnaireSettings(settings) {
  const titleEl = document.getElementById('questionnaireTitle');
  if (titleEl && settings.title) {
    titleEl.textContent = settings.title;
  }

  const introEl = document.getElementById('questionnaireIntroText');
  if (introEl && settings.intro_text) {
    renderMultilineText(introEl, settings.intro_text);
  }

  const startBtn = document.getElementById('startQuestionnaire');
  if (startBtn && settings.start_button_text) {
    startBtn.textContent = settings.start_button_text;
  }

  const completionEl = document.getElementById('questionnaireCompletionText');
