...
async function startParticipantSession(customId) {
  ...
  if (customId && customId.trim()) {
    ...
    // 也尝试通知后端（不阻塞）
    apiPost('/participants/start', { external_id: trimmedId }).catch(function (e) {
...
async function saveQuestionnaireResponse(data) {
  ...
  // 尝试提交到后端（不阻塞前端流程 — 用 Promise 而非 await）
  const body = {
  ...
  apiPost('/questionnaire/response', body).catch(function (e) {
...
async function completeQuestionnaire() {
  ...
  // 不阻塞前端流程
  apiPost('/questionnaire/complete', {
...
async function completeGame() {
  ...
  if (participantId) {
    apiPost('/game/complete', {
      participant_id: participantId
    }).catch(function (e) {
      console.warn('[storage] 后端计分通知失败:', e.message);
    });
  }
