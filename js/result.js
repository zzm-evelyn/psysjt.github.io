...
async function renderReportActions(currentGameKey) {
  ...
  if (nextIndex !== -1) {
    actions.innerHTML = '<button class="btn btn-success btn-lg" onclick="goToNextGame(' + nextIndex + ')">进入下一个情景游戏</button>';
    return;
  }

  actions.innerHTML = [
    '<button class="btn btn-primary btn-lg" onclick="navigateTo(\'result.html\')">查看报告汇总</button>',
    '<button class="btn btn-secondary btn-lg" style="margin-left:8px;" onclick="navigateTo(\'index.html\')">返回首页</button>'
  ].join('');
}
...
function renderFinalAction() {
  ...
  actions.innerHTML = '<button class="btn btn-secondary" onclick="navigateTo(\'index.html\')">返回首页</button>';
