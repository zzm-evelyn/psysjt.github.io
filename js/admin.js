...
async function showParticipantDetail(participantId) {
  ...
  try {
    ...
    if (data.questionnaire_responses && data.questionnaire_responses.length > 0) {
      ...
      html += '<table class="admin-table"><thead><tr><th>题目</th><th>选项</th><th>原始分</th><th>最终分</th><th>维度</th></tr></thead><tbody>';
      data.questionnaire_responses.forEach(r => {
        html += '<tr><td>' + esc(r.question_id) + '</td><td>' + esc(r.selected_option_text || '') + '</td><td>' + (r.raw_score || 0) + '</td><td>' + (r.final_score || 0) + '</td><td>' + esc(r.dimension_name || '-') + '</td></tr>';
      });
